...
- In the `claw-kit` repo, `.claw/project.json` no longer carries legacy `memory.embedding.local.modelCacheDir = ".claw/models"`; project-local `.claw/models` can still appear as a fallback or recovery cache surface, but it is no longer a required pinned config contract.
...
- Active ONNX cache refreshes for `Snowflake/snowflake-arctic-embed-m-v2.0` are stable when run serially with `CLAW_EMBEDDING_LOCAL_DEVICE=cpu`; parallelizing `claw search --query` with `claw search index --refresh` right after a cache delete can trip `Load model ... system error number 13`.
...
- `scripts/install-cli.ps1` 已在本机刷新全局 CLI，`npm list -g @veewo/claw --depth=0` 现在显示 `0.1.34`，`(Get-Command claw).Source` 指向 `C:\Users\chany\AppData\Roaming\npm\claw.ps1`，并且 `claw --help` 运行成功。
...
- `sync-latest-remote-and-refresh-local-installs` 把“先同步到最新 `origin/main`，再刷新全局 `claw` CLI 与本地 Codex plugin cache，并复核版本与路径”固化为工作站 closeout 规则。
- `refresh-local-embedding-onnx-cache` 把 `.claw/models/Snowflake/snowflake-arctic-embed-m-v2.0` 视为可重建缓存，并把“重建后避免并发 `search` + `search index --refresh`，必要时串行 `CLAW_EMBEDDING_LOCAL_DEVICE=cpu` rescue refresh”固化为本地恢复规则。
...
- 当前 workspace/package 版本线已经推进到 `0.1.35`：`package.json`、`package-lock.json`、`packages/core/package.json`、`packages/cli/package.json`、`packages/openclaw-adapter/package.json`、`packages/codex-adapter/package.json` 与 `packages/codex-adapter/.codex-plugin/plugin.json`（`0.1.35+codex.20260612132837`）完成对齐，`CHANGELOG.md` 也补上了 0.1.35 的 `delegateSubagents` field-by-field dispatch 说明。- `@veewo/claw-core@0.1.35` and `@veewo/claw@0.1.35` were both published successfully; the maintainer machine refreshed `@veewo/claw@0.1.35`, and the local Codex plugin cache landed at `0.1.35+codex.20260612132837`.