...
- Apply the same bounded evolution model to Truth and ADR written by the built-in writer. `knowledgeWriter.datedSectionsToKeep` is the only mechanical size control and defaults to `6` complete dated sections per changed canonical document. It is a direct `knowledgeWriter` option; the old nested `knowledgeWriter.retention` shape is not supported. An external `knowledgeWriter.externalSkill` owns its own governance and skips this snapshot/compaction automation.
