...
- Keep `create-claw-skill` entry routing free of storage scope. Template-backed creation outside a `.claw` project requires the caller to select `--scope session` explicitly; otherwise Core returns its scope-decision guidance, while plain plan creation keeps project initialization semantics.
