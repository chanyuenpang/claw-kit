...
`claw-kit` 需要确认 Codex hook 的正确职责边界。hook 可以在 session lifecycle 上提供启动增强，但它不是 canonical harness correctness 的唯一基础，也不是完整的 thread orchestration API。

本地验证已经确认：

- `SessionStart` hook 适合做 startup bootstrap enhancement
...
- `SessionStart` hook 通过 `packages/codex-adapter/hooks/session-start-bootstrap.mjs` 运行
...
- Codex session start 获得 attach-free bootstrap enhancement，同时不要求 hook 接管主流程
