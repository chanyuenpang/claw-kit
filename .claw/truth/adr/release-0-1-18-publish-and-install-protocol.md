...
- 正式 publish 完成后，除了安装烟测，还要刷新并验证 global CLI 与当前 active Codex identity 对应的 marketplace source/cache，确保 npm 包与实际加载面都切到新发布版本
- 正式 publish 完成后，还要直接核对 registry `dist-tags.latest` / `version`、active identity、marketplace source manifest 与 matching cache manifest，避免“包已发布或存在更新 cache，但实际 enabled plugin 仍停留在旧版本”的假完成态
...
- release closeout 的 done 条件继续包括本机全局 `claw` CLI 刷新、当前 enabled Codex identity 的 source/cache 刷新、关键缓存文件与 source snapshot 一致，并把 cache target 固定到本次 release 对应的插件 manifest 版本
- Codex cache 刷新必须 identity-aware：`claw-kit@claw-kit` active 时验证 official repository marketplace source 与 official cache；`claw-kit@claw-kit-local` 只有在它实际 enabled 时才验证 local source/cache。development cache 较新但未启用不能作为 release completion 证据
...
- `0.1.58` 这轮把 release closeout 的一个新分支固定下来：如果发布的目的之一是单独验证 `autoUpdate` / update-first 行为，可以刻意把本地 CLI 和 plugin cache 刷新留到后续验证回合，但发布本身仍必须完成双包 publish 和 registry verification，不能因为暂缓本地刷新就把 closeout 视为未完成
...
- active identity 现在决定哪一组 Codex source/cache 构成 release 证据，避免未启用的 development cache 制造假阳性
...
- 0.1.58 的 release closeout 采用了“先发布并核验 registry，再把本地刷新留给独立 auto-update 验证流”的做法，因此本地 CLI / plugin cache 是否已更新不再是这一轮发布收口的必要条件
