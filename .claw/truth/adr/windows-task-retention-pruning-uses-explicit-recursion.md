...
异步 `adr-writer` 还要求 completed plan 在 foreground `claw plan done` 返回后继续保持原路径可读；立即移动当前 task directory 会让 writer 首次读取与归档发生竞态。
...
- foreground closeout 不必等待异步 ADR deposition，同时 writer 在一小时窗口内仍可从原 `planPath` 读取 completed plan。
