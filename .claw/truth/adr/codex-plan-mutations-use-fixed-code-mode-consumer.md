...
0.1.72 将 Goal 目标状态保证纳入 `schema-v2 ensure_goal` consumer/driver 合同。Goal 状态判断因此必须继续由固定程序路径负责，不能退回 Agent 判断，也不能为兼容旧调用轨迹恢复 direct-call 或 split-call fallback。
...
- `schema-v2 ensure_goal` 由固定 consumer/driver 负责把 Goal 请求收敛到目标状态；Agent 不判断当前或先前 Goal 状态，也不重放旧 Goal 调用轨迹。
...
- `schema-v2 ensure_goal` 消除 Goal 桥接对 Agent 所见历史状态的依赖；source 与 versioned cache 中的 consumer/driver 必须保持该合同一致。
