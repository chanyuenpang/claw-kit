...
`claw-kit` 的主工作流已经收敛到 `using-claw-kit` -> complexity gate -> `planning` -> `workflowGuidance`，而低复杂度、无需正式 plan 分解的 round 仍然需要一个兼容旧流程的直达入口。
...
- 低复杂度绕行路径不会再和 `claw search` / `workflowGuidance` 的正式 workflow 语义混淆
