...
Codex workflow 把 `claw plan write` 作为先绑定 task scope 的入口；当项目上下文 recall 有帮助时，再在 `claw plan write` 之后运行 `claw search`，并把结果补回当前 plan / execution flow。
...
- planning flow 先绑定 canonical task scope，再检索项目上下文回填计划或执行路径，减少无 scope recall 和重复发现。
