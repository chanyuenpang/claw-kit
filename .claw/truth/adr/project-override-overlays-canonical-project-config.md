...
- `workflow.goalMode.enabled` 的 canonical 默认值是 `true`。
- `workflow.truthDispatch.mode` 的 canonical 默认值是 `per_task`。
...
- 当 effective config 设置 `workflow.goalMode.enabled=false` 时，workflow guidance 不再返回 `goalMode`。
- 当 effective config 设置 `workflow.truthDispatch.mode=final_only` 时，workflow guidance 不再返回 mid-task `truth-writer` delegation，但完成期沉淀仍然保留。
...
- 用户可以只覆盖局部 nested config，而不必复制整份 `.claw/project.json`。
