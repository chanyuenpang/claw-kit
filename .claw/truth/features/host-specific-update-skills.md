...
- If refreshing the CLI leaves an already-created plan bound to an older installed `TEMPLATE.json` and the new CLI rejects a remaining canonical mutation, preserve the plan and template identity unchanged. The handoff requires both the matching published CLI and a readable file at the plan's exact persisted `templateFile`: core resolves an explicit template file directly, so a marketplace refresh that removes the older versioned cache makes the mutation fail before canonical state changes even when the older CLI is available. Do not substitute the refreshed template or workspace content for the retained contract. Run only the remaining mutations through the fixed Codex driver after the exact published template is available at the retained path, then verify that the global `claw` command still resolves to the target version. This compatibility recovery does not authorize unpublished workspace content, a local marketplace, or keeping the older CLI installed.
...
- `persisted templateFile removed by marketplace refresh`
