...
- Knowledge finalization worker 只异步消费进入 `end.*` 的 source `plan.json`、其相邻 report、finalize id 与 job host。它按 `externalSkills` 的顺序动态运行 job 配置的 writer skill；列表缺失或为空时选择 consistency-aware `claw-kit:knowledge-writer`。内置 pass 共同维护 Truth 和 ADR、收敛每条 material current claim 的唯一 owner。外部 skill 的 prompt 与治理边界由 `external-writer-skill-config.md` 唯一拥有；所有配置的 skill 成功完成后才请求 recall indexing。
...
- 主 agent 不再判断或派发 knowledge writer（qoder host 例外：其 in-conversation subagent-dispatch 链路由 `qoder-knowledge-finalization-flow.md` 拥有）；combined writer 的返回文本不控制 fixed Truth→ADR deposition sequence，异步知识采集也不能反向接管 plan lifecycle 或 session binding。
