...
- Background policy 的 Stop capture 仍要求存在非空的最终 assistant message：`packages/cli/src/cli.ts` 的 `runStopHook(...)` 在缺少 message 时直接返回，`packages/core/src/knowledge-sidecar.ts` 的 `tryCaptureKnowledgeStop(...)` 也会拒绝空 message。只有成功追加最终 report entry 后，sidecar 才会消费 `pendingTurnOwner` 并创建 background `KnowledgeFinalizationJob`。Codex subagent policy 不使用这条触发链：终态 mutation 已创建 `reportCapture.mode = "claim"` 的 ready job，Stop 不捕获、排队、启动或修改该 job。
...
- Background policy 的 Codex `Stop` 从 transcript 尾部定位当前 Stop turn 的边界，只读取该轮的完整记录而不扫描更早轮次。每个成功 `task.done` 返回关联同 turn 内位于它之前且距离最近的 assistant message；没有可靠前置结论时跳过。sidecar 把所有合格结论作为 `entryType: "task_conclusion"` 追加到 Stop registry 当前拥有的相邻 report，再追加本 turn 的最终 report entry，不按 plan 或 task identity 过滤。Subagent policy 在 `knowledge claim --project-root <path> --finalize-id <id>` 的原子 prepare 阶段复用同一 transcript extractor，把终态 mutation 前已经存在的 task conclusions 冻结到 report 后才授予 claim ownership。
...
- Knowledge finalization executor 只异步消费进入 `end.*` 的 source `plan.json`、其相邻 report、finalize id、job host、冻结的 writer 配置，以及仅在 `memory.autoUpdate` 有效时冻结的 external-document path snapshot。`externalSkills` 非空时按顺序物化真实 skill assignments；列表缺失或为空时物化 Core 内部的 consistency-aware governance assignment。内置 assignment 共同维护 Truth 和 ADR、收敛每条 material current claim 的唯一 owner，但不以用户可发现的 `claw-kit:knowledge-writer` skill 形式发布。冻结的 external-document stage 始终追加在所选 writer assignments 之后，通过内部 `doc-updater` session subplan 只治理已有文档；外部 skill 的 prompt 与治理边界由 `external-writer-skill-config.md` 唯一拥有。全部 assignments 成功完成后才请求 recall indexing。
...
- `knowledgeWriter.executionPolicy` 只选择 executor launcher：`background` 是默认值，由 Stop 后启动独立 host agent；Codex `subagent` 的终态 plan mutation 在 Stop 前直接创建 ready job 并返回确定性 `knowledgeDispatch`，主 agent 直接 `spawn_agent` 后结束当前回合且绝不等待。background CLI worker看到 `subagent` job 会直接退出，不得 claim 或跨策略 fallback。
- Codex subagent executor 消费 `packages/core/resources/delegate-writer/TEMPLATE.json` 所定义的内部 session workflow：直接以 `projectRoot + finalizeId` 执行 `knowledge claim`，claim 在授予 ownership 前冻结父 transcript 中已有的 task conclusions，并原子返回 `jobPath`、`claimToken` 与动态 assignment template；随后 executor 创建 session-scoped assignment subplan、依序完成全部 assignments，最后凭 token 调用一次 `knowledge done`。Cindy 使用相邻的 `cindy-delegate-writer/TEMPLATE.json` 建立同样由模板拥有 claim、assignment subplan 与 terminal acknowledgement 的 session lifecycle；差异只在 Ghost operation 与 originating report capture，而不是绕过 plan/subplan 直接执行 prompts。
- `knowledge wait` 保持 readiness-only 且不建立、读取或验证 executor binding，但不属于 ready-job Codex subagent delegate 的当前执行序列。session scope 来自内部 delegate template；`claim` 与 `done` 都是 session-agnostic 的 token lifecycle。`done` 只接受 active claim token 与匹配的终态结果，assignment template 在成功或失败终态后清理。
...
- 内置 governance 在动态 assignment template 中进入 guidance-backed fixed-order workflow，依次完成材料结论提取、证据新鲜度判断、canonical owner 搜索、Truth 维护、ADR 维护和跨文档一致性审查；没有 external-document stage 时共六任务，存在冻结 doc-updater assignment 时会在 ADR 与终末一致性审查之间插入第七项。writer 按内容解释所有提供的材料，不把 plan、report、closeout 命名或任何固定字段、记录形状、序列化格式当作输入 schema；source `plan.json`、相邻 report 与 finalize id 只是当前 finalizer 提供的具体 runtime inputs。task status 只解释 completed、pending 与 blocked scope，task 标题、描述、requirements 与 intentions 不能被提升为执行结果。writer 固定先维护 Truth、再维护 ADR，不设置 `truth` / `adr` / `both` / `noop` route task。
...
- `packages/core/src/knowledge-sidecar.ts`：writer config snapshot、Codex subagent ready-job 创建、claim-time report capture、Truth/ADR Markdown 编码归一化、report 路径 containment 与幂等 finalization-result 写入。
...
- `packages/cli/src/cli.ts`：Codex-facing CLI lifecycle 与 hook entry；background policy 在 Codex `Stop` 时把 transcript conclusions 交给 sidecar，subagent policy 在 `knowledge claim` prepare 中冻结已有 conclusions。
...
- 不能把 background policy 的 final message 当作可随意裁掉的重复文本；空 message 会让该 policy 的 report capture 和终态 job 创建同时短路。Codex subagent 已改为 terminal ready job 与 claim-time conclusion capture，不能再把它描述为等待 Stop 创建 job。
...
- `subagent` policy 下主 agent仍不得等待 writer；job 已经 ready，但 foreground closeout 不应被异步治理反向阻塞。Stop worker 必须忽略该 policy 的 job，background worker也不得抢先 claim；两条 launcher 路径保持无跨策略 fallback。
...
- Background final-message capture 回归必须覆盖空 message 不创建 job、非空 message 才消费 `pendingTurnOwner` 并创建幂等 job；Codex subagent 回归必须覆盖终态 mutation 直接创建 ready job、Stop 不捕获或修改它，以及 claim 冻结已有 task conclusions 后才签发 token。
...
- 内置 real-worker 验收必须确认内部 delegate template 建立 session scope、guidance-backed 固定顺序 completion、可选 doc-updater 的插入位置、Truth/ADR 与 affected external docs 协同审查、项目 canonical 输出边界，以及没有递归 finalization。外部 writer 验收应确认动态 skill prompt、无内置治理注入、无治理快照或 dated trimming，并在 external-document governance 有效时仍追加同一 doc assignment；不完整 assignment subplan 仍失败。
