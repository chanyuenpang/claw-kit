...
- 当前恢复存在两条独立的同步缺口。`SessionStart` 的 `auto-claw` 路径经 `runContextCommand()`、`tryResolveActiveWorkflowSnapshot()` 与 `buildSessionStartAdditionalContext()` 只把恢复的 workflow snapshot 渲染为文本；它不会调用 `buildHostActions()` 或固定 code-mode driver，因此恢复 active plan 时不会派发原生 `create_goal` 或 `update_plan`。显式 `plan resume` 会走 mutation 的 host-action 路径，但若等待前的 task 已是 `in_progress`，`buildCodexPlanProjection()` 在 `process.wait` 和恢复后的 `process.active` 都优先保留该 task 为 `in_progress`，投影相同，`codexPlanProjectionChanged()` 因而省略 `update_plan`。这两处都是当前已定位、尚未修复的限制；不要把恢复 prompt 或 resumed Goal action 误当作进度已同步。
