...
- `.claw/project.json` 以 `knowledgeWriter` 对象保存 combined writer 配置：`externalSkill`、`model`、`reasoningEffort` 与 `datedSectionsToKeep`。`externalSkill = null` 选择内置 `claw-kit:knowledge-writer`；`datedSectionsToKeep` 的治理语义由 `truth-and-adr-corpus-semantics.md` 唯一拥有。
...
- worker prompt 动态使用 `knowledgeWriter.externalSkill` 指定的 skill；外部 skill 只接收 skill invocation、source plan、相邻 report、finalization id 和通用输入解释边界，不注入内置 writer 的 one-owner、Truth → ADR、retention 或其他语义治理规则。
