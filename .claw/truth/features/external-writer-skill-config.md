...
- `packages/core/src/workflow-guidance.ts` 生成 `workflowGuidance.delegateSubagents` 时会返回显式 `skill` 字段和显式 `model: "gpt-5.6-luna"`（Luna），这样 Codex 适配器不需要再从上下文猜测 writer 绑定。
