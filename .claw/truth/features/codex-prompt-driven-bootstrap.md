...
- `plan write` now records a project-level session binding from the current `sessionKey` to a `.claw`-relative `planPath`; active workflow identity no longer lives in per-task `ownerSessionKey` metadata.
