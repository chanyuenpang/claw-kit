...
- `packages/core/src/plan.ts` 的 `createSubplan(...)` 派生 child plan file under `plans/`，再走 `writePlan(...)`；`writePlan(...)` 在有 `parentTaskId` 时写入 `parentPlan` / `parentTaskId`，把 parent task 的 `execution.type` 标记为 `subplan`，记录 `execution.subplan` / `execution.planPath`，并把 task meta 的 `activePlan` 切到 child plan。
...
- `packages/core/src/plan.ts` 的 `editPlan(...)` 收到 `subplanClosureCandidate` 后调用 `completeSubplanAndResumeParent(...)`，把 parent task 标记 done，将 `activePlan` 恢复到 parent plan，并基于 parent plan 返回下一步 `workflowGuidance`。
...
- 同一测试文件还覆盖直接 `writePlan(... parentTaskId ...)` 时 child plan 落在 `plans/`、parent task execution 标记为 `subplan`、task meta `activePlan` 切到 child plan。
