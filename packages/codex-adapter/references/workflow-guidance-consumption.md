...
Code-mode consumption is the adapter execution method. The CLI-provided versioned driver is the single distributed runtime consumer; the adapter repository keeps a non-distributed test oracle only for contract tests. The driver enforces an explicit stage-aware allowlist, accepts schema v1 native `create_goal` or `update_goal`, and consumes each action exactly once. Before a Goal mutation, the program inspects `get_goal`: `create_goal` executes only when there is no unfinished Goal. When an active, blocked, or unknown unfinished Goal already exists, the driver preserves it and returns a visible recovery note without mutating Goal state. Goal updates follow an explicit transition matrix: `blocked` is allowed only from `active`, while `complete` is allowed only from `active` or `blocked`; a missing, already complete, unknown, or otherwise mismatched Goal consumes the action as a no-op. The agent must never inspect Goal state through a separate `get_goal` call. The driver and test oracle do not parse host error wording or invent compensation. A recovered active session runs `plan sync` once through the bridge to create a missing root-plan Goal without disturbing an existing one. Unknown schemas fail closed. Codex has no separate host-call fallback, and Codex compact results do not return `goalMode` or `goalTool`.
...
- all tasks done: write retrospective and key decisions, then run `claw plan done`.
