...
- `knowledgeWriter.externalSkill`
  - optional combined truth-and-ADR writer override; `null` uses `claw-kit:knowledge-writer`
- `memory.enabled`
...
- `truthDispatch`, `per_task`, and `final_only` are removed; Codex auto-doc always judges the completed plan report at closeout.
