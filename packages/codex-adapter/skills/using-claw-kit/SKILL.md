...
- current plan status, if available
...
8. If `workflowGuidance.goalMode` appears and this thread has no conflicting active goal, set the thread goal from `workflowGuidance.goalMode.recommendedObjective`.
...
- prefer delegating it to a `researcher` specialist to preserve main-agent context
- dispatch the specialist as `explorer`
- attach `claw-kit:researcher` explicitly
- reuse an existing same-type `researcher` in the thread when available
...
- if `.claw/project.json` says `gitnexus.enabled = true`, have the researcher discover and use GitNexus-oriented capabilities when they improve code investigation
...
- Use a lighter/faster model for writer specialists by default: prefer `gpt-5.4-mini` unless the task clearly needs a stronger model.
- Attach the corresponding writer skill explicitly when dispatching, such as `claw-kit:truth-writer` or `claw-kit:adr-writer`.
- Reuse an existing same-type specialist when available.
...
- ADR deposition is async and should not block main-agent wrap-up.
...
Hooks are optional enhancement, not the correctness path.

- `claw hook <event>` first resolves cwd to a `.claw` project.
- Outside `.claw` projects, hooks return `skipped=true` and do not write project-specific logs.
- The reliable path remains prompt-driven startup plus CLI `workflowGuidance`.
