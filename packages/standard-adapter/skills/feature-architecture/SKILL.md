...
---
<!-- AUTO-GENERATED from shared/skills/feature-architecture/SKILL.md. Edit the shared source instead. -->
...
- 主代理触发本技能时，先通过恢复入口 `claw context` 取得 context（标准 hostless 形态：不附加 `--host`，导出 `CLAW_SESSION_ID`），只以返回的 `activeWorkflow` 判断当前会话是否已有绑定的 claw task；不得扫描或猜测其他 task。
