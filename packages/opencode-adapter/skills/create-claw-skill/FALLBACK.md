...
1. Inspect the source skill or distill the idea into its trigger, ordered workflow, tools, constraints, real branches, verification, and lifecycle handoff. Put each task-specific constraint in its owning sub-task's `detail`. Keep only cross-task constraints in top-level `rules`. If an existing `TEMPLATE.json` has no `version` or a version older than the current CLI, review and optimize the full package against the current contract instead of only bumping the field. Use `guidance.onPlanStart` only for a genuine discussion-to-execution delivery point; `claw plan start` is optional global syntax sugar, not a required template step.
