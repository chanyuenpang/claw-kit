...
For explicit non-claw project initialization, a visible `claw context` invocation is enough.
...
4. If `claw context` succeeded, treat the project as initialized for claw workflow entry.
...
- Do not tell the user to run `claw init` first when the task is explicit project initialization.
