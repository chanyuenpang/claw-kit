...
function stringValue(args: Record<string, unknown>, ...keys: string[]): string | undefined {
  const value = firstValue(args, ...keys);
  return typeof value === "string" ? value : undefined;
}

function stringValues(args: Record<string, unknown>, ...keys: string[]): string[] | undefined {
  const value = firstValue(args, ...keys);
...
function referenceValues(args: Record<string, unknown>): unknown[] | undefined {
...
function assertKnownArgs(operation: string, args: Record<string, unknown>, keys: readonly string[]): void {
...
function planFieldUpdates(args: Record<string, unknown>): Record<string, unknown> {
  const updates: Record<string, unknown> = {};
  const assignText = (target: string, ...keys: string[]) => {
    const value = stringValue(args, ...keys);
    if (value !== undefined) updates[target] = value;
  };
  const assignStrings = (target: string, ...keys: string[]) => {
...
function taskDoneEntry(value: unknown): unknown {
...
export function daemonInput(operation: string, args: Record<string, unknown>): unknown {
  const str = (...keys: string[]): string | undefined => stringValue(args, ...keys);
  const arr = (...keys: string[]): unknown[] | undefined => {
    const value = firstValue(args, ...keys);
