...
Closeout is required after `claw plan done`. The hostless default resolves `knowledgeWriter.executionPolicy` to `main-agent`: run `claw knowledge prepare --source agent-memory --project-root <path>`, execute the returned assignments from your own memory, then `claw knowledge complete --source agent-memory --project-root <path> --config-fingerprint <hash>`. With an explicit `background` policy, run the three-step chain instead: inline capture (`claw internal-knowledge-capture` with stdin JSON) → dispatch (`claw internal-knowledge-dispatch --job <jobPath>`) → execute the session-scoped writer plan to its terminal transition. Never add `--host`.
