...
test("Codex entry stays compact without dropping guidance, lifecycle, or the mutation bridge", async () => {
  ...
  assert.match(skill, /argv:\s*\["plan",\s*"create",\s*"<title>"\]/i);
...
test("Codex plugin exposes an explicit same-agent knowledge-capture skill", async () => {
  ...
  const cliPackage = JSON.parse(await fs.readFile(new URL("../cli/package.json", adapterRoot), "utf8"));
  assert.deepEqual(runtime, { schemaVersion: 1, package: "@veewo/claw", version: cliPackage.version });
